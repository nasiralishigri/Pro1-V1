...
contract('this is the trabic Crowdsale ',function([_,wallet,invester1,invester2],value,rate){
    ...
    describe('Whitelisted CrowdSale',function(){    // check
      ...
      // it("is this address is whitelisted ", async function(){

      //   await wait(2000);
      //   var isWhitelisted = await this.trabicCrowdSale.isWhitelisted(invester1);
      ...
      // })
        it('reject non whitelisted user ', async function(){
          await wait(1000);
            ...
            const nonWhiteListed =  invester2;
            await this.trabicCrowdSale.buyTokens(nonWhiteListed, { value: ether(1), from: nonWhiteListed}).should.be.rejectedWith(EVMRevert);
    
        })
        it('success transaction whitelisted user ', async function(){
          await wait(1000);
    
            const nonWhiteListed =  invester1;
            await this.trabicCrowdSale.buyTokens(nonWhiteListed, { value: ether(1), from: nonWhiteListed}).should.be.fulfilled;
...
/////    Check user is successfully added to whitelisted or not //////
